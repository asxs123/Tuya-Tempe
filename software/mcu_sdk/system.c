...
static void weather_data_raw_handle(const unsigned char p_data[], unsigned short data_len)
{
    ...
    if(p_data[0] != 1 || data_len < 1) {
        //接收失败
			OLED_ShowStr(70,1,"jssb",16);
    }else {
        if(data_len < 4) {
            //数据为空
					OLED_ShowStr(70,1,"sjwk",16);
        }
        while (i < data_len) {
            can_len = p_data[i];   //参数名长度
            OLED_ShowStr(70,1,"ok",16);
            my_memset(can, '\0', 15);
            //my_memcpy(can, p_data + i + 1, can_len - 2);
						my_memcpy(can, p_data + i + 1, can_len);
