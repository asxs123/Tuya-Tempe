...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_SAMPLING,当前传感器采样时间设置); //VALUE型数据上报;
   ...
   */
    if(time[0] == 1) {
        //正确接收到wifi模块返回的本地时钟数据
				FlashBuffer.Timer_Hour_Value = time[4];
				FlashBuffer.Timer_Min_Value = time[5];
				FlashBuffer.Timer_Sec_Value = time[6];
...
void weather_open_return_handle(unsigned char res, unsigned char err)
{
    ...
    if(res == 1) {
        //打开天气返回成功
			OLED_ShowStr(0,1,"test",16);
    }else if(res == 0) {
        //打开天气返回失败
        //获取错误码
        err_num = err; 
			OLED_ShowStr(0,1,"err",16);
    }
}
 ...
 * @note   MCU需要自行实现该功能
 */
void weather_data_user_handle(char *name, unsigned char type, const unsigned char *adata, char day)
{
    ...
    if(type == 0) { //参数是INT型
				...
				{
					value_int = adata[2] << 8 | adata[3];
				}
        //value_int = adata[0] << 24 | adata[1] << 16 | adata[2] << 8 | adata[3];
    }else if(type == 1) {
        my_strcpy(value_string, adata);
    }
    
    //注意要根据所选参数类型来获得参数值！！！
    if(my_strcmp(name, "w.temp") == 0) {
				FlashBuffer.out_temper = adata[3];
        //printf("day:%d temp value is:%d\r\n", day, value_int);          //int 型
    }else if(my_strcmp(name, "w.humidity") == 0) {
				FlashBuffer.out_humidity = value_int;
        //printf("day:%d humidity value is:%d\r\n", day, value_int);      //int 型
    }else if(my_strcmp(name, "pm25") == 0) {
				FlashBuffer.out_aqi = value_int;
