...
void wifi_uart_service(void)
{
    ...
    while((rx_in - offset) >= PROTOCOL_HEAD) {
        if(wifi_data_process_buf[offset + HEAD_FIRST] != FRAME_FIRST) {  //帧头第一字节验证
            offset ++;
            continue;
        }
        
        if(wifi_data_process_buf[offset + HEAD_SECOND] != FRAME_SECOND) {//帧头第二字节验证
            offset ++;
            continue;
        }  
        
        if(wifi_data_process_buf[offset + PROTOCOL_VERSION] != MCU_RX_VER) {//帧协议版本号验证
            offset += 2;
            continue;
        }      
        
        rx_value_len = wifi_data_process_buf[offset + LENGTH_HIGH] * 0x100; //数据长度
        rx_value_len += (wifi_data_process_buf[offset + LENGTH_LOW] + PROTOCOL_HEAD);////数据长度
        if(rx_value_len > sizeof(wifi_data_process_buf) + PROTOCOL_HEAD) {  //判断数据是否超出
            offset += 3;
            continue;
        }
        
        if((rx_in - offset) < rx_value_len) {  //判断数据是否完整
            break;
        }
